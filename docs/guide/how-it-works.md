...
| **`HasGreasedSerialization`** | Eliminates the Carbon parse-and-reformat round-trip for date serialization — when a per-class probe certifies the output is a byte-identical rewrite. Also short-circuits the `toArray()` circular-recursion guard (a `debug_backtrace` + `Onceable` hash vanilla runs on *every* call) when no relations are loaded — there's nothing to recurse into, so `toArray()` is exactly `attributesToArray()`. **−27% on a relation-less `get()->toArray()`.** |
